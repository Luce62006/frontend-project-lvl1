import engine from '../index.js';
import getRandomNum from '../utils.js';

const description = 'What number is missing in the progression?';
const progressionOfNum = () => {
  let ferstNum = getRandomNum(2, 7);

  const diffbetwenNum = getRandomNum(2, 5);
  const progressionOfNum = [];
  for (let i = 0; i < 10; i += 1) {
    ferstNum += diffbetwenNum;
    progressionOfNum.push(ferstNum);
  }
  const indexOfHiddenNum = getRandomNum(0, 9);
  const correctAnswear = progressionOfNum.splice(indexOfHiddenNum, 1, '..');
  return progressionOfNum;
};

const getQuestionAndCorrectAnswer =()=>{
  const question = progressionOfNum(); 
  const progressionGameInfo = [question, correctAnswer];
  return progressionGameInfo;
};

export default ()=>engine(description, getQuestionAndCorrectAnswer);
